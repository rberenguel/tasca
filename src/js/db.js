...
export const dbOps = {
    check: () => { if (!db) throw new Error("Database loading..."); },
    ...
    update: (task) => new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_NAME, 'readwrite');
        const store = tx.objectStore(STORE_NAME);
    ...
    delete: (uuid) => new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_NAME, 'readwrite');
        const store = tx.objectStore(STORE_NAME);
    ...
    get: (uuid) => new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_NAME, 'readonly');
        const store = tx.objectStore(STORE_NAME);
    ...
    updateProject: (projData) => new Promise((resolve, reject) => {

...
export const getChecklistMembers = async (parentUuid, dbOps, statusFilter = null) => {
...
export const handleChecklist = async (ctx) => {
  ...
  // Resolve parent
  const parentUuids = await resolveRefs(parentIdArg, ctx.displayMapRef, ctx.dbOps)
  ...
  // Resolve members
  const memberUuids = await resolveRefs(memberIdArg, ctx.displayMapRef, ctx.dbOps)

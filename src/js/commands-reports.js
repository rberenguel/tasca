...
const buildContextFilter = (ctx) => {
  ...
  return (t) => {
    ...
    for (const tag of fTags) {
      if (!hasVirtualTag(t, tag) && !t.tags?.includes(tag.substring(1)))
        return false;
    }
...
export const handleReport = async (subCmd, subArgs, print, dbOps, projects) => {
  if (!subCmd || subCmd === "help") {
    print(
      `<div class="msg-standalone"><b>Report Commands:</b>
  ...
  <span class="cmd">report audit [period]</span> - Recurring task skip rate analysis (default: 4w)</div>`,
      false,
    );
    return true;
  }
  ...
  if (subCmd === "rot") {
    ...
    const refProjects = new Set(
      projects
        .filter((p) =>
          p.tags?.some((t) => ["reference", "ref"].includes(t.toLowerCase())),
        )
        .map((p) => p.name),
    );
    ...
    const pending = all
      .filter(
        (t) =>
          t.status === "pending" && (!t.project || !refProjects.has(t.project)),
      )
      .map((t) => ({
        ...t,
  ...
  if (subCmd === "done") {
    ...
    for (const arg of subArgs) {
      if (arg.startsWith("by:")) {
        ...
        if (["p", "pro", "proj", "project"].includes(val)) groupBy = "project";
  ...
  if (subCmd === "cycle" || subCmd === "slo") {
    ...
    if (completed.length === 0) {
      print('<span class="msg-info">No completed tasks found.</span>');
      return true;
    }
  ...
  if (subCmd === "churn") {
    ...
    const activeTasks = all.filter(
      (t) => (t.start && t.start >= cutoff) || (t.end && t.end >= cutoff),
    );
  ...
  if (subCmd === "audit") {
    ...
    const skippedCount = resolved.filter((t) => t.status === "skipped").length;
    const skipRate =
